<template>
  <div id="trailWatch" class="trail-watch" :class="{ active_a: !isActive }">
    <div class="wathc-packup">
      <img class="packup" @click="animate" v-if="isActive" src="~sysStatic/images/control_left@2x.png" alt="" />
      <img class="packup" @click="animate" v-else src="~sysStatic/images/right@2x.png" alt="" />
    </div>
    <div class="watch-title">
      <p class="watch-title-item-one">实时监控</p>
      <p class="watch-title-item-two">当前视野内显示设备数量 {{ nowCounts }}</p>
    </div>
    ...
    <template>
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="title" :render-header="deviceTypeRenderHeader">
            ...
            <div class="watch-table-list">
              <div v-if="scope.row.online === 1">
                <span
                  ...
                  class="card-icon"
                ></span>
                <span class="table-item-title" style="marginleft: 50px; color: #020202">{{ scope.row.title }}</span>
              </div>
              <div v-else>
                <span
                  :style="{
                  ...
                  class="card-icon"
                ></span>
                <span class="table-item-title" style="marginleft: 50px; color: #606275">{{ scope.row.title }}</span>
              </div>
              <span v-if="scope.row.online === 1" style="color: #020202">在线</span>
...
export default {
  ...
  created() {
    ...
    Object.keys(DeviceType).forEach((item) => {
      item > 0 && this.checkListOpts.push(DeviceType[item]) && this.checkList.push(DeviceType[item]);
    });
  },
  mounted() {
    // this.isOnline = "";
                ...
                on: {
                  change: (value) => {
                    if (value) {
                      ...
                      const i = this.checkList.findIndex((val) => val === item);
