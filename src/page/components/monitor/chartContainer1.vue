<template>
  <div class="panelStyle" v-show="['origin', 'both', ''].includes(trailType)">
    <div ref="lineChart" class="echartsStyle" @mouseup="touchs"></div>
    <div ref="timeChart1" class="echartsStyle1" v-if="showTimeline">
...
import iconPlay from '@/assets/images/trailActionsImg/icon_play@2x.png';
import iconPause from '@/assets/images/trailActionsImg/icon_pause@2x.png';
import { mapState } from 'vuex';
import LbsTimeline from '../../example/heatAnalysis/lbsTimeline';
...
export default {
  ...
  data() {
    return {
      ...
      timelineChartOpt: {
        ...
        label: {
          show: false,
          formatter: (value, index) => moment(parseInt(value)).format('mm:ss'),
        },
        playInterval: 1000,
        lineStyle: {
        ...
        controlStyle: {
          ...
          showPrevBtn: false,
          showPlayBtn: true,
          // playIcon: 'image://data:image/gif;base64,',
          //  playIcon: `image://${iconPause}`,
          // stopIcon: `image://${iconPlay}`
          // borderWidth: 0
        },
  ...
  watch: {
    ...
    'state.isPlaying': {
      handler(newVal, oldVal) {
        ...
        this.option2.baseOption.tooltip.triggerOn = newVal ? 'none' : 'mousemove|click';
  ...
  methods: {
    ...
    drawTimeChart(id) {
      ...
      timeChart = echarts.init(this.$refs[id]);
      timeChart.setOption(this.baseOption);
    },
    drawLineChart(id) {
    ...
    drawChart(id) {
      ...
      charts.on('timelinechanged', (params) => {
        ...
        this.option2.baseOption.visualMap.pieces[0] = {
          gte: this.xAxisData[0],
          lte: this.xAxisData[this.xAxisData.length - 1],
          color: '#0000FF',
          colorAlpha: 1,
        };
        //  更改指示器的指示线值
        this.option2.options[this.currentIndex].xAxis[0].axisPointer.value = timelineDataArr[this.currentIndex];
        // 更改时间轴跳转间隔
    ...
    concatOption() {
      ...
      this.lineChartOpt.visualMap[0].pieces = [
        {
          gt: this.xAxisData[0],
          lte: this.xAxisData[(this.xAxisData.length / 2).toFixed()],
          color: '#4E90FF',
          colorAlpha: 1,
        },
        {
          gt: this.xAxisData[(this.xAxisData.length / 2).toFixed()],
          lte: this.xAxisData[this.xAxisData.length],
          color: 'rgb(156,194,255)',
          colorAlpha: 1,
        },
      ];
      this.timelineChartOpt.data = [...this.xAxisData];
      this.baseOption.timeline.data = [...this.xAxisData()];
    },
    speedChange() {
