...
export default {
    ...
    methods: {
        ...
        getCurrentZeroClockTime(time) {
            ...
            return (
                new Date(Number(time) * 1000 + 28800000).setHours(0, 0, 0, 0) /
                1000
            ).toString();
        },
        ...
        getNextZeroClockTime(time) {
            return (
                new Date(Number(time) * 1000 + 28800000 + 86400000).setHours(
                    0,
                    0,
                    0,
                    0
        ...
        getTimePara(){
            ...
            if (typeof this.$store.state.safeAnalysis.timePicker === "object") {
                ...
                var timePicker_time = String(
                    this.getCurrentZeroClockTime(
                        this.$store.state.safeAnalysis.timePicker.getTime() /
                            1000 -
                            86400
                    )
                );
            } else {
                var timePicker_time = this.getCurrentZeroClockTime(
                    this.$store.state.safeAnalysis.timePicker
                );
            }

            let current_time = this.getCurrentZeroClockTime(
            ...
            if (this.$store.state.safeAnalysis.isFromHistory) {
                ...
                this.time.push(
                    this.getTime(),
                    this.getCurrentZeroClockTime(historyTime),
                    this.getNextZeroClockTime(historyTime)
                );
            } else if (timePicker_time !== current_time) {
                // 如果时间选择器选择的是以往的时间
                this.time.push(
                    this.getTime(),
                    this.getCurrentZeroClockTime(timePicker_time),
                    this.getNextZeroClockTime(timePicker_time)
                );
            } else {
                // 如果时间选择器选择的是当日的时间
        ...
        questEvent(event_id) {
            ...
            this.getTimePara();
            this.$API.safeAnalyze
                .eventDetail(event_id, ...this.time)
                .then((res) => {
                    this.loading = false;
                    var out_res = res;
    ...
    watch: {
        ...
        "$store.state.safeAnalysis.timePicker": function (newVal) {
            ...
            this.time.push(
                this.getTime(),
                this.getCurrentZeroClockTime(newVal),
                this.getNextZeroClockTime(newVal)
            );
            this.$API.safeAnalyze.allEventsPoints(...this.time).then((res) => {
                this.$store.state.safeAnalysis.loading_traffic = false;
                let geo = [];
                res.data.forEach((ele) => {
                    geo.push({
