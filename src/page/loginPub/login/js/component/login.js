...
var TPL_TEL = [
    ...
    '<div class="item-input"><span class="icon-user"></span><input type="text" style="text-indent: 32px;height: 42px;" name="mb" placeholder="请输入手机号码"><span class="tip"></span></div>',
    '</div>',
    '<div class="form-item">',
    // '<div class="item-title">验证码</div>',
    '<div class="item-input">',
    '<span class="icon-password"></span>',
    '<input type="text" name="code" style="width:60%; text-indent: 32px;height: 42px;" placeholder="请输入验证码">',
    ...
    '<button type="submit" class="actSubmit" style="height: 40px;" disabled>登 录</button>',
    '</div>',
    '</div>',
    ...
    submit: function (e) {
        ...
        $.ajax({
                ...
                if (res.info && res.info.error === 0) {
                    ...
                    $.ajax({
                        url: HOST + 'gov/console/has_auth',
                        type: 'GET',
                        dataType: 'jsonp',
                        crossDomain: true, // 设置跨域为true
                        xhrFields: {
                            withCredentials: false // 默认情况下，标准的跨域请求是不会发送cookie的
                        },
                    }).done(function (res) {
                        ...
                        if (res.detail.auth == 0) {
                            ...
                            $.ajax({
                                url: HOST + `gov/console/app_auth/list?output=json`,
                                type: 'get',
                                dataType: 'json',
                                xhrFields: {
                                    withCredentials: false // 默认情况下，标准的跨域请求是不会发送cookie的
                                ...
                                console.log(res)
                                if (res.info.error == 0) {
                                    let arr = res.detail.apps.map(val => {
                                        return val.id
                                    })
                                    // TODO: 轨迹平台权限控制
                    ...
                    alert(res.info && (res.info.msg == "验证失败" ? "验证码不正确" : res.info.msg) || '登录失败，请刷新重试');
                    _target.html('登录')
                    _target.removeAttr('disabled')
                }
            })
