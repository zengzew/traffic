...
export default {
    ...
    computed: {
        ...
        isDark() {
            return this.$store.state.theme.indexOf("dark") >= 0
                ? "dark"
                : "light";
        },
    },
    watch: {
        ...
        $route() {
            if (this.$route.name === "dangerAnalyze") {
                this.$store.state.auth.navList =
                    this.$store.state.auth.AiCalPlatformNavList;
            } else if (this.$route.name === "monitor") {
                this.$store.state.auth.navList =
                    this.$store.state.auth.defaultNavList;
            }
    ...
    methods: {
        ...
        isAllNavShow() {
            ...
            return (
                pathArr.includes(this.$route.path) ||
                this.$route.path === "/trail"
            );
        },
        openMenu() {
